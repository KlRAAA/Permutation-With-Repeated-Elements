function generateOutfits() {
  ...
  const accessories = document
    ...
    .value.split(",")
    .map((item) => item.trim().toLowerCase());
  const filter = document.getElementById("filter").value.toLowerCase(); // Ensure filter is case-insensitive
  ...
  for (let top of tops) {
    for (let bottom of bottoms) {
      for (let shoe of shoes) {
        let outfit = `${top} + ${bottom} + ${shoe}`;
        if (accessories.length > 0 && accessories[0] !== "") {
          for (let accessory of accessories) {
            let finalOutfit = `${outfit}${accessory ? " + " + accessory : ""}`;
            if (
              filter === "all" ||
              (filter === "casual" &&
              ...
              listItem.textContent = finalOutfit;
